...
func flattenClusterProxyConfigV2(d *schema.ResourceData, in *ClusterProxyConfigV2) error {
	if in == nil {
		return fmt.Errorf("flattening cluster proxy config: Input unstructured is nil")
	}
...
func expandClusterProxyConfigV2(in *schema.ResourceData) (*ClusterProxyConfigV2, error) {
	if in == nil {
		return nil, fmt.Errorf("expanding cluster proxy config: Input ResourceData is nil")
	}
