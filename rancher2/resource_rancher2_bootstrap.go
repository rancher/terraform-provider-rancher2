...
logged:
	for {
		for _, pass := range loginPass {
			if len(pass) > 0 {
				tokenID, token, err = DoUserLogin(meta.(*Config).URL, meta.(*Config).ProxyURL, bootstrapDefaultUser, pass, bootstrapDefaultTTL, bootstrapDefaultSessionDesc, meta.(*Config).CACerts, meta.(*Config).Insecure)
				if err == nil {
					break logged
				}
			}
