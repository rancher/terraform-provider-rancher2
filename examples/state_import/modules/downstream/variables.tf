...
variable "zone" {
  type        = string
  description = <<-EOT
    The DNS domain zone to add the new Rancher cluster to.
    eg. "example.com" in "rancher.example.com"
  EOT
}
variable "domain" {
  type        = string
  description = <<-EOT
    The DNS domain name to add the new Rancher cluster as in the zone.
    eg. "rancher" in "rancher.example.com"
  EOT
}
...
variable "rancher_token" {
  type        = string
  description = <<-EOT
    The Rancher authentication token to use.
  EOT
}
variable "kubeconfig_path" {
  type        = string
  description = <<-EOT
    The path to the cluster's kubeconfig file.
    This is used to patch the machine config to allow for temporary credentials.
  EOT
}
