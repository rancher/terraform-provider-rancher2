...
locals {
  ...
  wrknc             = var.worker_node_count
  cpnc              = var.control_plane_node_count
  node_count        = (local.wrknc + local.cpnc)
...
resource "rancher2_machine_config_v2" "core" {
  ...
  generate_name = "core-${local.cluster_name}"
  amazonec2_config {
    ami            = local.ami_id
...
resource "rancher2_machine_config_v2" "worker" {
  ...
  generate_name = "worker-${local.cluster_name}"
  amazonec2_config {
    ami            = local.ami_id
    region         = local.aws_region
    security_group = [local.downstream_security_group_name]
...
resource "rancher2_cluster_v2" "rke2_cluster" {
  ...
  rke_config {
    ...
    machine_pools {
      name               = "${local.cluster_name}-wk"
      control_plane_role = false
      etcd_role          = false
      worker_role        = true
      quantity           = local.wrknc
      machine_config {
