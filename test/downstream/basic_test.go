...
type DeployDownstreamOutput struct {
	kubeconfig            string `json:kubeconfig`
	address               string `json:address`
	admin_password        string `json:admin_password`
	downstream_kubeconfig string `json:downstream_kubeconfig`
}

var Attributes = []string{"kubeconfig", "address", "admin_password", "downstream_kubeconfig"}
...
func TestDownstreamBasic(t *testing.T) {
	...
	directory := "deploy_downstream_basic"
...
func CheckOutputs(t *testing.T, terraformOptions *terraform.Options) {
	for i := range Attributes {
		output, err := terraform.OutputRequiredE(t, terraformOptions, Attributes[i])
		if err != nil {
			t.Fail()
