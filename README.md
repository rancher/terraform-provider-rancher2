...
If you wish to work on the provider, you'll first need [Go](http://www.golang.org) installed on your machine (version 1.25+ is *required*). A Nix flake is also available in the repo which is used to generate an environment for build and test.
...
The run_tests script will run the examples to test.
...
* the `release/v14` branch with v14.0.0+ is aligned with Rancher 2.14
* the `release/v15` branch with v15.0.0+ is aligned with Rancher 2.15
