...
func flattenClusterRKEConfigCloudProviderAzure(in *managementClient.AzureCloudProvider, p []interface{}) ([]interface{}, error) {
	...
	if in.CloudProviderBackoffDuration > 0 {
		obj["cloud_provider_backoff_duration"] = int(in.CloudProviderBackoffDuration)
	}

	if in.CloudProviderBackoffExponent > 0 {
		obj["cloud_provider_backoff_exponent"] = int(in.CloudProviderBackoffExponent)
	}

	if in.CloudProviderBackoffJitter > 0 {
		obj["cloud_provider_backoff_jitter"] = int(in.CloudProviderBackoffJitter)
	}

	if in.CloudProviderBackoffRetries > 0 {
		obj["cloud_provider_backoff_retries"] = int(in.CloudProviderBackoffRetries)
	}

	obj["cloud_provider_rate_limit"] = in.CloudProviderRateLimit

	if in.CloudProviderRateLimitBucket > 0 {
		obj["cloud_provider_rate_limit_bucket"] = int(in.CloudProviderRateLimitBucket)
	}
	...
	if in.MaximumLoadBalancerRuleCount > 0 {
		obj["maximum_load_balancer_rule_count"] = int(in.MaximumLoadBalancerRuleCount)
	}
...
func expandClusterRKEConfigCloudProviderAzure(p []interface{}) (*managementClient.AzureCloudProvider, error) {
	obj := &managementClient.AzureCloudProvider{}
	if len(p) == 0 || p[0] == nil {
		return obj, nil
	}
	...
	if v, ok := in["cloud_provider_backoff_duration"].(int); ok && v > 0 {
		obj.CloudProviderBackoffDuration = int64(v)
	}

	if v, ok := in["cloud_provider_backoff_exponent"].(int); ok && v > 0 {
		obj.CloudProviderBackoffExponent = int64(v)
	}

	if v, ok := in["cloud_provider_backoff_jitter"].(int); ok && v > 0 {
		obj.CloudProviderBackoffJitter = int64(v)
	}

	if v, ok := in["cloud_provider_backoff_retries"].(int); ok && v > 0 {
		obj.CloudProviderBackoffRetries = int64(v)
	}
	...
	if v, ok := in["cloud_provider_rate_limit_bucket"].(int); ok && v > 0 {
		obj.CloudProviderRateLimitBucket = int64(v)
	}
	...
	if v, ok := in["maximum_load_balancer_rule_count"].(int); ok && v > 0 {
		obj.MaximumLoadBalancerRuleCount = int64(v)
	}
