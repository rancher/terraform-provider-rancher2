...
const nilInputErrMsg = "Input setting is nil"
...
func flattenPodSecurityAdmissionConfigurationTemplateExemptions(in *managementClient.PodSecurityAdmissionConfigurationTemplateExemptions) []interface{} {
	...
	if len(in.Usernames) > 0 {
		obj["usernames"] = toArrayInterface(in.Usernames)
	}

	if len(in.RuntimeClasses) > 0 {
		obj["runtime_classes"] = toArrayInterface(in.RuntimeClasses)
	}

	if len(in.Namespaces) > 0 {
		obj["namespaces"] = toArrayInterface(in.Namespaces)
	}
...
func flattenPodSecurityAdmissionConfigurationTemplate(d *schema.ResourceData, in *managementClient.PodSecurityAdmissionConfigurationTemplate) error {
	if in == nil {
		return fmt.Errorf("[ERROR] flattening pod security admission configuration template: %s", nilInputErrMsg)
	}
...
func expandPodSecurityAdmissionConfigurationTemplateDefaults(in []interface{}) *managementClient.PodSecurityAdmissionConfigurationTemplateDefaults {
	...
	if v, ok := m["audit"].(string); ok && len(v) > 0 {
		obj.Audit = v
	}

	if v, ok := m["audit_version"].(string); ok && len(v) > 0 {
		obj.AuditVersion = v
	}

	if v, ok := m["enforce"].(string); ok && len(v) > 0 {
		obj.Enforce = v
	}

	if v, ok := m["enforce_version"].(string); ok && len(v) > 0 {
		obj.EnforceVersion = v
	}

	if v, ok := m["warn"].(string); ok && len(v) > 0 {
		obj.Warn = v
	}

	if v, ok := m["warn_version"].(string); ok && len(v) > 0 {
		obj.WarnVersion = v
	}
...
func expandPodSecurityAdmissionConfigurationTemplate(in *schema.ResourceData) (*managementClient.PodSecurityAdmissionConfigurationTemplate, error) {
	if in == nil {
		return nil, fmt.Errorf("[ERROR] expanding pod security admission configuration template: %s", nilInputErrMsg)
	}
