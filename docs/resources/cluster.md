...
* `imported_config` - (Optional) The imported configuration for generic imported Clusters. Conflicts with `oke_config`, `rke2_config` and `k3s_config` (list maxitems:1)
...
* `release_channel` - (Optional) The GKE release channel to enroll the cluster in. One of `Stable`, `Regular`, `Rapid` or `Extended`. 
