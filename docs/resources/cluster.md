...
* `aep` - (Optional) Attachable entity profile (string)
* `apic_hosts` - (Optional) List of APIC hosts to connect for APIC API (list)
* `apic_refresh_time` - (Optional) APIC refresh time in seconds (string)
* `apic_user_crt` - (Optional) APIC user certificate (string)
* `apic_user_key` - (Optional) APIC user key (string)
* `apic_user_name` - (Optional) APIC user name (string)
...
* `extern_dynamic` - (Optional) Subnet to use for dynamic external IPs (string)
* `enable_endpoint_slice` - (Optional) Whether to enable endpoint slices (string)
* `encap_type` - (Optional) Encap type: vxlan or vlan (string)
...
* `kube_api_vlan` - (Optional) The VLAN used by the physdom for nodes (string)
* `l3out` - (Optional) L3out (string)
* `l3out_external_networks` - (Optional) L3out external networks (list)
* `max_nodes_svc_graph` - (Optional) Max nodes in service graph (string)
* `mcast_range_end` - (Optional) End of mcast range (string)
* `mcast_range_start` - (Optional) Start of mcast range (string)
* `no_priority_class` - (Optional) Whether to use priority class (string)
* `node_subnet` - (Optional) Subnet to use for nodes (string)
...
* `node_svc_subnet` - (Optional) Subnet to use for service graph (string)
* `service_monitor_interval` - (Optional) Service monitor interval (string)
* `service_vlan` - (Optional) The VLAN used by LoadBalancer services (string)
...
* `extern_static` - (Optional)  Subnet to use for static external IPs (string)
* `subnet_domain_name` - (Optional) Subnet domain name (string)
* `system_id` - (Optional) ACI system ID (string)
* `tenant` - (Optional) ACI tenant (string)
* `token` - (Optional) ACI token (string)
...
* `vrf_name` - (Optional) VRF name (string)
* `vrf_tenant` - (Optional) VRF tenant (string)
