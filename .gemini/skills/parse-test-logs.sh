...
# Usage: .gemini/skills/parse-test-logs.sh [options]
...
Examples:
  ...
  $ .gemini/skills/parse-test-logs.sh

  # Parse a specific log file with color suppressed
  $ .gemini/skills/parse-test-logs.sh -f /tmp/my_test.log --no-color

  # Show failures only
  $ .gemini/skills/parse-test-logs.sh --failed-only
EOF
}
