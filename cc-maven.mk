...
TEST_TARGETS += mvn-verify codecov-upload
...
export MAVEN_OPTS += -Dmaven.repo.local=$(CI_BIN)/m2

# enable CI profile for spotbugs, test-coverage, and dependency analysis
MAVEN_PROFILES += jenkins
endif

# Use local `mvnw` if present in the repo, else fallback to globally installed `mvn`
MVN := $(GIT_ROOT)/mvnw
ifeq ($(wildcard $(MVN)),)
MVN := mvn
endif
...
mvn-docker-package: docker-login
	$(MVN) package \
	        $(MAVEN_SKIP_CHECKS) \
