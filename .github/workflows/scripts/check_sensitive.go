...
func matchesKeywords(name string, keywords []string) bool {
	...
	if strings.HasSuffix(lower, "_id") || strings.HasSuffix(lower, "_name") {
		return false
	}
...
func checkGoFile(fset *token.FileSet, file *ast.File, filePath string, ignoreList *IgnoreList, keywords []string) []string {
	...
	ast.Inspect(file, func(n ast.Node) bool {
		...
		for _, elt := range compLit.Elts {
			...
			// Check if Sensitive: true is defined and if it is a string type
			hasSensitive := false
			isTypeString := false
			for _, structElt := range structLit.Elts {
				...
				if ident.Name == "Type" {
					if sel, ok := structKV.Value.(*ast.SelectorExpr); ok {
