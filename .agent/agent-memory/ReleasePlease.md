...
`release-please-action` automates CHANGELOG generation, GitHub release creation, and version bumps by parsing git history for Conventional Commits (`fix:`, `feat:`, `feat!:`). It maintains a running Release PR containing the updated changelog and version bumps. Upon merge of the Release PR, release-please tags the commit and creates a draft GitHub Release. Subsequently, GoReleaser compiles, GPG-signs, and publishes the signed assets directly into the pre-created draft Release.
