...
get_latest_version() {
  ...
  if ! latest_version=$(run_with_retry curl -s "https://registry.terraform.io/v1/modules/${module_name}" 2>/dev/null | jq -r '.version // empty' 2>/dev/null); then
    return 1
  fi
...
update_all_modules() {
  ...
  local modules
  modules=$(grep -E -o 'source[ \t]*=[ \t]*"[A-Za-z0-9_-]+/[A-Za-z0-9_-]+/[A-Za-z0-9_-]+"' -r --exclude-dir={.git,.terraform,node_modules,tf_plugin_cache} --include="*.tf" "${search_dir}" 2>/dev/null \
    | sed -E 's/.*source[ \t]*=[ \t]*"([^"]+)".*/\1/' \
    | sort -u || echo "")
  ...
  for mod in ${modules}; do
    ...
    # Use pure numeric version (no 'v' prefix) as conventional in Terraform Registry modules in this repo
